...
const GameSection = ({ onBook ,searchQuery}) => {
  const [gameTypes, setGameTypes] = useState([]);
  const [open, setOpen] = useState(false);
  const [selectedContent, setSelectedContent] = useState(null);

  useEffect(() => {
    axios.get('http://localhost:8081/new/game')
      .then(response => {
        setGameTypes(response.data);
      })
      .catch(error => {
        console.error("There was an error fetching the game types!", error);
        toast.error('Error fetching game types. Please try again.');
      });
  }, []);
            ...
            <Grid item xs={12} sm={6} md={4} key={index}>
              <Card sx={{ bgcolor: '#C1C8E4', height: '100%', borderRadius: 2 }}>
                ...
                <CardContent>
                  ...
                  <Button color="primary" variant="contained" sx={{ mt: 1 }} onClick={() => onBook(game.title,game.price)}>Book</Button>
                </CardContent>
              </Card>
            </Grid>
          ))}
