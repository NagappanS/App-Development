...
const DjSection = ({ onBook,searchQuery }) => {
  const [djStyles, setDJStyles] = useState([]);
  const [open, setOpen] = useState(false);
  const [selectedContent, setSelectedContent] = useState(null);

  useEffect(() => {
    axios.get('http://localhost:8081/new/dj')
      .then(response => {
        setDJStyles(response.data);
      })
      .catch(error => {
        console.error("There was an error fetching the DJ styles!", error);
        toast.error('Error fetching DJ styles. Please try again.');
      });
  }, []);
  ...
  const filteredDj = djStyles.filter(dj => 
    dj.title.toLowerCase().includes(searchQuery)
  );
  return (
    <>
            ...
            <Grid item xs={12} sm={6} md={4} key={index}>
              <Card sx={{ bgcolor: '#90AEAD', height: '100%', borderRadius: 2 }}>
                ...
                <CardContent>
                  ...
                  <Button color="primary" variant="contained" sx={{ mt: 1 }} onClick={() => onBook(dj.title,dj.title)}>Book</Button>
                </CardContent>
              </Card>
            </Grid>
          ))}
